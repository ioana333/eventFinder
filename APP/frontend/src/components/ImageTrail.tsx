...
// Folosim 'any' pentru eveniment pentru a acoperi ușor MouseEvent și TouchEvent
...
class ImageItem {
  ...
  constructor(DOM_el: HTMLElement) {
    ...
    this.rect = this.getRect(); // Inițializăm rect aici
  ...
  getRect(): DOMRect {
    return this.DOM.el.getBoundingClientRect();
  }
}
...
export default function ImageTrail({ items = [], variant = 1 }: ImageTrailProps) {
  ...
  return (
    <div className="content w-full h-full" ref={containerRef}>
      {items.map((url, i) => (
